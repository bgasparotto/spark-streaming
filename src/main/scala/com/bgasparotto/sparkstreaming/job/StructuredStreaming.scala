...
object StructuredStreaming {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder
      ...
      .master("local[*]")
      .config("spark.sql.streaming.checkpointLocation", "spark_tmp/checkpoint")
      .getOrCreate()

    setupLogging()

    // Create a stream of text files dumped into the logs directory
    // After it starts and processes the same batch, duplicate the log file in order to observe
    // the streaming happening
    val rawData = spark.readStream.text("dataset/apache")
    ...
    // (instead of "append").
    val query =
      windowed.writeStream.outputMode("complete").format("console").start()
