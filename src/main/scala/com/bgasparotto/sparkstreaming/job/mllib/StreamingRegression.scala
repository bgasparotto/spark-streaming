package com.bgasparotto.sparkstreaming.job.mllib
...
/** Example of using streaming linear regression with stochastic gradient descent.
  ...
  * Train data: `nc -kl 9999 < dataset/linear-regression/regression.txt`
  * Test data: `nc -kl 7777 -i 1 < dataset/linear-regression/regression.txt`
  */
object StreamingRegression {
  ...
  def main(args: Array[String]) {
    ...
    val ssc =
      new StreamingContext("local[*]", "StreamingRegression", Seconds(1))
    ...
    val testingLines =
      ssc.socketTextStream("127.0.0.1", 7777, StorageLevel.MEMORY_AND_DISK_SER)
