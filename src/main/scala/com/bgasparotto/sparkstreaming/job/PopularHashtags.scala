...
object PopularHashtags {
  ...
  def main(args: Array[String]) {
    ...
    val lowerCaseHashtags = hashtags.map(_.toLowerCase)

    // Map each hashtag to a key/value pair of (hashtag, 1) so we can count them up by adding up the values
    val hashtagKeyValues = lowerCaseHashtags.map(hashtag => (hashtag, 1))
    ...
    val sortedResults =
      hashtagCounts.transform(rdd => rdd.sortBy(x => x._2, ascending = false))
