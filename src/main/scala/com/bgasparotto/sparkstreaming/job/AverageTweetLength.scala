...
object AverageTweetLength {
  ...
  def main(args: Array[String]) {
    ...
    var shortestTweet = Int.MaxValue
    var longestTweet = Int.MinValue
    ...
    lengths.foreachRDD((rdd, _) => {
      ...
      if (count > 0) {
        ...
        shortestTweet = Math.min(shortestTweet, rdd.min())
        longestTweet = Math.max(longestTweet, rdd.max())

        println(
          "Total tweets: " + totalTweets.get() +
