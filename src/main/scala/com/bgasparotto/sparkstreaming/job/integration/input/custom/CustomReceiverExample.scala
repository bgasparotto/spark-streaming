...
/** Example from the Spark documentation; this implements a socket receiver from scratch using a
  * custom Receiver.
  *
  * Custom receiver docs at http://spark.apache.org/docs/latest/streaming-custom-receivers.html
  *
  * Run this class and publish the messages from the access log with:
...
class CustomReceiver(host: String, port: Int)
  ...
  def onStart() {
    ...
    val receiver = new Runnable() {
      override def run() { receive() }
    }
    new Thread(receiver, "Socket Receiver").start()
  }
  ...
  private def receive() {
    ...
    try {
      ...
      val inputStream = new InputStreamReader(socket.getInputStream, "UTF-8")
...
object CustomReceiverExample {
  ...
  def main(args: Array[String]) {
    ...
    val ssc =
      new StreamingContext("local[*]", "CustomReceiverExample", Seconds(1))
